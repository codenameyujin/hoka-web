(function ($) {
    ...
    if (prefix == 'destroy') {
      ...
      return $(this).filter('select:not(.clickybox-replaced)').addClass('clickybox-replaced').each(function () {
        ...
        setTimeout(function() {
          $optCont.find('li a').each(function () {
            ...
            if ($a.hasClass('active') && $a.hasClass('unavailable')) {
              let cartBuyNow = document.querySelector('[data-buy-cart-btn]');
              cartBuyNow.classList.add('disabled-true');
              let cartBuy = document.querySelector('[data-buy-now-empty-btn]');
              if(cartBuy) {
                cartBuy.classList.add('disabled-true');
              }
              let buyItNow = document.querySelector('[data-buy-now-cart]');
              if(buyItNow) {
                buyItNow.classList.add('disabled-true');
              }
            }
          });
        }, 0);
        ...
        $optCont.on('click.clickyboxes', 'a', function () {
          ...
          if ($(this).hasClass('active') && !$(this).hasClass('unavailable')) {
            let cartBuyNow = document.querySelector('[data-buy-cart-btn]');
            cartBuyNow.classList.remove('disabled-true');
            let cartBuy = document.querySelector('[data-buy-now-empty-btn]');
            if(cartBuy) {
              cartBuy.classList.remove('disabled-true');
            }
            let buyItNow = document.querySelector('[data-buy-now-cart]');
            if(buyItNow) {
              buyItNow.classList.remove('disabled-true');
            }
          }
  ...
  const CartForm = class extends HTMLElement {
    ...
    adjustItemQuantity(item, change) {
        ...
        fetch(`${window.Shopify.routes.root}cart/change.js`, {
          ...
          then((response) => {
            ...
            $.get(theme.routes.cart_url, function (data) {
              ...
              cartItem();
              $('.as-salomon__cartPage [data-item-property]').each(function (index, item) {
                const propertyString = $(item).attr('data-item-property');
                const obj = JSON.parse(propertyString);
                if (obj.config_image_urls.length > 0) {
                  const propertyImage = obj.config_image_urls[0];
                  $(item).find('.col-image img').attr('src', propertyImage);
                  $(item).find('.col-image img').attr('srcset', propertyImage);
                }
              });
            });
          }).
      ...
      if (document.body.classList.contains('template-cart')) {
        disableClick();
        membershipDiscount(customerID, 'cartpage');
      } else {
        disableClick();
  ...
  theme.ProductMediaGallery = function ($gallery, $thumbs, isFeaturedProduct, isQuickbuy, galleryId) {
    ...
    function initSwiper() {
      ...
      var swiperOpts = {
        ...
        freeMode: false,
        freeModeFluid: false,
        slidesPerView: mediaCount > 1 ? 1 : 1,
    ...
    if ($clickies.length > 0) {
      ...
      for (var optionIndex = 0; optionIndex < productData.options.length; optionIndex++) {
        ...
        for (var key in optionValues) {
          ...
          if (!optionValues[key]) {
            $('.selector-wrapper:eq(' + optionIndex + ') .clickyboxes li a', containerTwo).filter(function () {
      ...
      $pager.on('click', 'a', function (e) {
        ...
        $.get($(this).attr('href'), function (data) {
          ...
          $('[data-product-wishlist-id]').each(function (index, item) {
            ...
            var userID = parseInt($(item).data('customer-wishlist-id'), 10);
            if (productID && userID) {
    ...
    $formContainer.filter('[data-ajax-add-to-cart="true"]:not(.feedback-go_to_cart)').find('.product-purchase-form').on('submit', function (e) {
      ...
      $.post(shopifyAjaxAddURL, $form.serialize(), function (itemData) {
        ...
        if ($form.hasClass('feedback-add_in_modal') || $form.hasClass('feedback-add_in_modal_no_checkout')) {
          ...
          showThemeModal('', null);
          // Wait for a short delay to ensure the modal is rendered before adding the class
          setTimeout(() => {
  ...
  theme.OptionManager = new function () {
    ...
    _.updateQtySelectors = function (variant, $container) {
      try{
        const invCount = _._getVariantOptionElement(variant, $container).data('inventory');
        const qtyWrapper = $container.find('.quantity-wrapper')
        const qtyInput = qtyWrapper.find('input');
        const qtyInputVal = parseInt(qtyInput.val());
        if(qtyWrapper) {
          qtyWrapper.find('[data-quantity]').removeAttr('disabled');
          if(invCount <= qtyInputVal) {
            qtyInput.val(invCount < 1 ? 1 : invCount);
            qtyWrapper.find('[data-quantity="up"]').attr('disabled','');
          }
        }
    ...
    _.updatePrice = function (variant, $container, quantity) {
      ...
      const tierDiscountRate = $priceArea.data('discountedPercentage') || 0;
      const productDiscountRate = calculateDiscountRate(compareAtPrice, variantPrice);

      var $newPriceArea = $('<div>');
      appendDiscountInfo($newPriceArea, $priceArea, compareAtPrice, variantPrice, productDiscountRate, tierDiscountRate, quantity);

      if (variant.unit_price_measurement) {
        appendUnitPriceInfo(variant, quantity, $newPriceArea);
      }
    ...
    function appendDiscountInfo($newPriceArea, $priceArea, compareAtPrice, variantPrice, productDiscountRate, tierDiscountRate, quantity) {
      if (productDiscountRate > tierDiscountRate) {
        appendPriceSpan($newPriceArea, 'was-price compare-at-price', compareAtPrice * quantity);
        $newPriceArea.append(' ');
      ...
      if (tierDiscountRate > 0) {
        ...
        const discountAmount = basePrice * (tierDiscountRate / 100);
        const discountedPrice = basePrice - discountAmount;

        appendPriceSpan($newPriceArea, 'was-price', basePrice * quantity);
        appendPriceSpan($newPriceArea, 'current-price', discountedPrice * quantity);
      } else {
        appendPriceSpan($newPriceArea, 'current-price', variantPrice * quantity);
      }
    }
    ...
    function appendUnitPriceInfo(variant, quantity, $newPriceArea) {
      ...
      appendPriceSpan($newUnitPriceArea, 'unit-price__price', variant.unit_price * quantity);
    ...
    _.initProductOptions = function ($productForm) {
      ...
      $productForm.find(_.selectors.variantIdInputs).each(function () {
        ...
        $(this).on('change.themeProductOptions firstrun.themeProductOptions', function (e) {
          ...
          // update price
          _.updatePrice(variant, $container, 1);

          let $quantityButtons = document.querySelectorAll('[data-quantity]');

          $quantityButtons.forEach(button => {
            button.addEventListener('click', () => {
              let $quantityWrapper = button.parentElement;
              let $quantity = $quantityWrapper.querySelector('[name="quantity"]');
              setTimeout(() => {
                _.updatePrice(variant, $container, $quantity.value);
                // Set the same quantity value for all quantity inputs
                document.querySelectorAll('[name="quantity"]').forEach(q => {
          ...
          _.updateQtySelectors(variant, $container);
  ...
  theme.ProductTemplateSection = new function () {
    ...
    this.onSectionLoad = function (target) {
      ...
      document.addEventListener('openRestockPopup', () => {
        $.colorbox({
          inline: true,
          fixed: true,
          maxHeight: "80%",
      ...
      $(window).on(`cc-variant-updated.product-swatches${sectionUniqueId}`, (e, args) => {
        ...
        if ($swatchesContainer.length) {
          ...
          $swatchesContainer.find('label.color-labels').append(`<span class="cc-swatches__label">${$swatchesContainer.find('.active').text()}</span>`);
        }
      });
  ...
  theme.ProductTemplateCalendarSection = new function () {
    ...
    document.addEventListener('openLaunchCalendarPopup', () => {
      $.colorbox({
        inline: true,
        fixed: true,
        maxHeight: "80%",
    ...
    document.addEventListener('updateEventPopupData', () => {
      $('#cboxLoadedContent').html($('#event-notification-content > .notificationConfirm').html());
    });

    // Raffle Notification Popup
    document.addEventListener('openRafflePopup', () => {
      $.colorbox({
        inline: true,
        fixed: true,
        maxHeight: "80%",
    ...
    document.addEventListener('updateRafflePopupData', () => {
    ...
    this.functions = {
      ...
      toggleFilter: function () {
        ...
        if ($productFilter.hasClass('-in')) {
          ...
          nav.bar.fadeOut(false);
          $('[data-footer-reset-button]').css('display', 'none');
          $('body').css('overflow', 'auto');
        } else {
          // $filterBtn.text($filterBtn.data('close-text'));
          nav.bar.fadeOut(true);
          $('[data-footer-reset-button]').css('display', 'block');
      ...
      refreshSelects: function () {
        ...
        if (document.getElementById('Filter-filter.v.availability-0')) {
          document.getElementById('Filter-filter.v.availability-0').parentElement.style.display = 'none';
        }
      },
        ...
        this.currentAjaxLoadUrlFetch = $.get(url, function (data) {
          ...
          if (matchingTags) {
            $('input[name="filter.p.tag"]').each(function (index, value) {
              ...
              let tagsArray = matchingTags.split(',');
              let tagMatches = false;
              tagsArray.forEach(tag => {
                if (tag.trim() === inputValue.trim()) {
                  tagMatches = true;
                  return false;
                }
              });
          ...
          $(document).on('mouseenter', '[data-product-main-block]', function () {
            if ($(this).find('.as-collection__imageSwatchWrap').length > 0) { }
            else {
              $(this).find('.caption, .as-productColors, .as-collection__badgeWrap').css({
                opacity: 1,
                visibility: 'visible'
              });
    ...
    this.functions = {
      ...
      initSidebar: function () {
        ...
        this.pageContent.querySelectorAll('.section-faq-accordion .section-heading h2').forEach((heading, index) => {
          ...
          const anchor = "faq-" + JSON.stringify(label.toLowerCase()).replace(/[^a-zA-Z0-9\uAC00-\uD7AF\u1100-\u11FF\u3130-\u318F\uA960-\uA97F\uAC00-\uD7A3]+/g, '');
      ...
      handleIndexClick: function (e) {
        ...
        theme.viewport.scroll.to(e.currentTarget.getAttribute('href'), -1, 0, () => {
          this.isScrolling = false;
        });
      }
    };
  ...
  jQuery(function ($) {
    ...
    $(document).on('click', '#page-menu.nav-uses-modal .main-nav li.has-children > a', function () {
      ...
      if (theme.strings.back.length > 0) {
        $menuList.prepend(`<li class="main-nav__title" data-nav-title="${trimmedTitle}"><a href="#">${trimmedTitle}</a></li>`);
    ...
    $(document).on('click', '.quantity-wrapper [data-quantity]', function () {
      ...
      //check and disable quantity if input is equal to inventory 
      try{
        ...
        if ($form.length) {
          ...
          if($var_id){
            ...
            if(parseInt($qty.val()) >= max_qty ){
              $(this).closest('.quantity-wrapper').find('[data-quantity="up"]').attr('disabled','');
            }else{
              $(this).closest('.quantity-wrapper').find('[data-quantity="up"]').removeAttr('disabled');
            }
          } 
        }
      }catch(err){
    ...
    $(document).on('click', '[data-collection-sort-filter]', function (event) {
      let sortByFilter = $(this).closest('.as-collection__sortBy').find('[data-collection-sortby-form]');
      sortByFilter.toggleClass('hidden');
    });

    $(document).on('click', '[data-collection-sortby-form] label', function (event) {
      event.preventDefault();
      event.stopImmediatePropagation();

      let sortValue = $(this).find('input').val();

      $('.cc-accordion .cc-product-filter__sort-by input').each(function (index, sortItem) {
        let originalSortValue = $(sortItem).val();
        if (originalSortValue === sortValue) {
          $(sortItem).closest('label').trigger('click');
        }
      });
    });

    $(document).on('click', function (event) {
      if (!$(event.target).closest('[data-collection-sort-filter], [data-collection-sortby-form]').length) {
        $('[data-collection-sortby-form]').addClass('hidden');
      }
    });

    $(document).on('click', '[data-product-image-id] img', function () {
      let imageID = $(this).closest('[data-product-image-id]').attr('data-product-image-id');
      $(this).closest('[data-product-image-id]').siblings().removeClass('as-collection__activeItem');
      $(this).closest('[data-product-image-id]').addClass('as-collection__activeItem');

      let sizeElement = $(this).closest('.as-collection__itemWrap--color').next().find('[data-product-size-id]');
      sizeElement.each(function (index, size) {
        let sizeID = $(size).attr('data-product-size-id');
      ...
      let imageSRC = $(this).parent().data('product-image-url');
      let imageALT = $(this).attr('alt');
      $(this).closest('[data-product-main-block]').find('[data-product-image] .rimage-wrapper').empty();
      let newImg = $('<img>');
      newImg.addClass('rimage__image fade-in lazyautosizes lazyloaded');
      newImg.attr('src', imageSRC);
      newImg.attr('alt', imageALT);
      $(this).closest('[data-product-main-block]').find('[data-product-image] .rimage-wrapper').append(newImg);
      let slides = $(this).closest('[data-product-main-block]').find('[data-product-image] .product-block--slide');
      if (slides.length > 0) {
        slides.remove();
      }

      let productURL = $(this).parent().data('product-url');
      $(this).closest('[data-product-main-block]').find('[data-product-image], .caption').attr('href', productURL);
    });
    ...
    $(document).on('click', '[data-filter-close]', function () {
      const nav = theme.Nav();
      if ($('.cc-product-filter').hasClass('-in')) {
        nav.bar.fadeOut(false);
        $('.cc-product-filter').removeClass('-in');
        $('[data-footer-reset-button]').css('display', 'none');
        $('body').css('overflow', 'auto');
      }
    });
    ...
    $(document).on('change', '.as-salomon__cartPage [data-cart-main-checkbox]', function () {
      let isChecked = $(this).is(':checked');

      $('.as-salomon__cartPage [data-cart-checkbox]').prop('checked', isChecked).trigger('change');
      totalCount = isChecked ? calculateTotalCount() : 0;

      $('.as-salomon__cartPage [data-cart-count]').text(totalCount);
    });

    $(document).on('change', '.as-salomon__cartPage [data-cart-checkbox]', function () {
      if ($(this).is(':checked')) {
        let quantity = parseInt($(this).closest('.cart-item').find('.cart-item__quantity-input').val());
        totalCount += quantity;
      } else {
        let quantity = parseInt($(this).closest('.cart-item').find('.cart-item__quantity-input').val());
        totalCount -= quantity;
      ...
      let allChecked = true;
      $('.as-salomon__cartPage [data-cart-checkbox]').each(function () {
        if (!$(this).prop('checked')) {
          allChecked = false;
          return false;
        }
      });
      $('.as-salomon__cartPage [data-cart-main-checkbox]').prop('checked', allChecked);
    });
    ...
    $(document).on('click', '.as-salomon__cartPage [data-bulk-delete-button]', function () {
      let formData = {
        'updates': {}
      };

      if ($('.as-salomon__cartPage [data-cart-checkbox]:checked').length === 0) {
        alert('상품을 선택하신 후 삭제해주세요.');
        return;
      }

      $('.as-salomon__cartPage [data-cart-checkbox]:checked').each(function () {
        let productId = $(this).attr('data-item-variant-id');
    ...
    theme.Sections.register('product-template-calendar', theme.ProductTemplateCalendarSection, { deferredLoadViewportExcess: 200 });
    ...
    theme.Sections.register('cart-template-popup', theme.CartTemplateSection, { deferredLoad: false });
